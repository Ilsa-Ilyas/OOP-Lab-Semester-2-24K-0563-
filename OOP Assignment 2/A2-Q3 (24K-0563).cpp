...
public:
    ...
    virtual void move() {
        cout << "Vehicle " << vehicleID << " is moving in a generic way.\n";
    }
    ...
    bool operator==(const Vehicle& v) const {
        return speed == v.speed && capacity == v.capacity && efficiency == v.efficiency;
    }
...
public:
    ...
    void calculateRoute(const string& destination) override {
        cout << "Drone " << vehicleID << " plotting aerial path to " << destination
             << " (Max Altitude: " << maxAltitude << " ft)\n";
    }

    void move() override {
        cout << "Drone " << vehicleID << " is flying at high speed.\n";
    }

    double estimateDeliveryTime(double distance) override {
        return distance / (speed * 1.2);  // 20% faster
    }
...
public:
    ...
    void calculateRoute(const string& destination) override {
        cout << "TimeShip " << vehicleID << " engaging chrono-path to " << destination
             << " (±" << timeAccuracy << " years)\n";
    }

    void move() override {
        cout << "TimeShip " << vehicleID << " is shifting through time-space.\n";
    }
...
public:
    ...
    void move() override {
        cout << "HyperPod " << vehicleID << " is speeding underground through " << tunnel << ".\n";
    }
...
void resolveConflict(const Vehicle& v1, const Vehicle& v2, double distance, double weight, const string& urgency) {
    ...
    if (urgency == "High") score1 += v1.efficiency * 0.2;

    double score2 = (v2.speed / distance * 0.5) + (v2.capacity / weight * 0.3);
    if (urgency == "High") score2 += v2.efficiency * 0.2;
...
int main() {
    ...
    // Comparison
    cout << "\n--- Vehicle Comparison ---\n";
    cout << (drone == timeship ? "Drone == TimeShip: Yes\n" : "Drone == TimeShip: No\n");
